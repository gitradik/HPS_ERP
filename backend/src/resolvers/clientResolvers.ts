...
const clientResolvers = {
  Query: {
    ...
    client: async (_: unknown, { id }: { id: number }): Promise<Client | null> => await Client.findByPk(id),
    activeClients: async (): Promise<Client[]> => await Client.findAll({ where: { isActive: true }, include: User }),
    clientByUserId: async (_: unknown, { userId }: { userId: number }): Promise<Client | null> => {
      return await Client.findOne({ where: { userId } });
    },
  },
  Mutation: {
